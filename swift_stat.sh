...
################################################################################
# Simple Nagios plugin to monitor swift stat                                   #
...
time="$(/bin/date +%s)"
swift_output="$($SWIFT stat 2>&1)"
if [[ $? -gt 0 ]]; then
   echo "SWIFT STAT CRITICAL - Command failed please check the logs at /var/log/icinga/swift_check.log"
   echo "$(/bin/date)" >> /var/log/icinga/swift_check.log
   echo "$nova_output" #>> /var/log/icinga/swift_check.log
   exit $STATE_CRITICAL
fi
